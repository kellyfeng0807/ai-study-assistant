...
def get_heatmap():
    ...
    for i in range(days - 1, -1, -1):
        ...
        # 2. 从 study_progress 获取 Error Book 复习/练习时间
        cur.execute('''
            SELECT COALESCE(SUM(review_time_minutes + practice_time_minutes), 0) 
            FROM study_progress WHERE date = ?
        ''', (date_str,))
        study_progress_minutes = cur.fetchone()[0] or 0
        ...
        # 4. 确定最终学习时间
        # 优先级: module_usage > study_progress > 估算
        if module_minutes > 0:
            study_minutes = module_minutes
        elif study_progress_minutes > 0:
            study_minutes = study_progress_minutes
