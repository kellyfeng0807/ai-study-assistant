...
let isTracked = false; // 防止重复发送

// 发送学习时间到后端
async function trackStudyTime(mode = "review", isCorrect = 0) {
    if (isTracked) return;
    ...
    try {
        ...
        isTracked = true;
        console.log(`📊 Tracked ${seconds}s for ${subject} (${mode})`);
    } catch (err) {
...
// 页面离开时发送时间
window.addEventListener('beforeunload', () => trackStudyTime("review", 0));
window.addEventListener('pagehide', () => trackStudyTime("review", 0));
