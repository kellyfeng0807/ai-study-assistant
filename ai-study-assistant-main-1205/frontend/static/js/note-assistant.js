...
class NoteAssistantManager {
    ...
    showEmptyState() {
        ...
        notesGrid.innerHTML = `
            <div class="empty-state" style="grid-column: 1 / -1; text-align: center; padding: 40px; color: #666;">
                <i class="fas fa-sticky-note" style="font-size: 48px; margin-bottom: 16px; opacity: 0.5;"></i>
                <h3 style="margin-bottom: 8px;">No notes yet</h3>
                <p>Record audio or enter text to generate your first note!</p>
            </div>
        `;
    }
    ...
    // Render all notes in the grid (without limit) - 保留兼容
